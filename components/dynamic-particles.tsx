...
export function DynamicParticles({ scrollYProgress, mouseX, mouseY }: DynamicParticlesProps) {
  ...
  const particleCount = useTransform(scrollYProgress, [0, 1], [100, 200])
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      particles.current = []
      const initialCount = Math.min(Math.floor(window.innerWidth * 0.05), 100)
